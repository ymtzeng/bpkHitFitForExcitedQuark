//
// $Id: TopGluon_Fit.cc,v 1.3 2012/02/03 06:47:44 ymtzeng Exp $
//
// File: src/TopGluon_Fit.cc
// Purpose: Handle jet permutations.
// Created: Jul, 2000, sss, based on run 1 mass analysis code.
//
// XXX handle merging jets.
// XXX btagging for top+gluon.
//
// CMSSW File      : src/TopGluon_Fit.cc
...
TopGluon_Fit_Args::TopGluon_Fit_Args (const Defaults& defs)
//
// Purpose: Constructor.
//
// Inputs:
...
bool TopGluon_Fit_Args::print_event_flag () const
//
...
//bool TopGluon_Fit_Args::do_TopGluon_flag () const
//
...
double TopGluon_Fit_Args::jet_mass_cut () const
//
...
double TopGluon_Fit_Args::mwhad_min_cut () const
//
...
double TopGluon_Fit_Args::mwhad_max_cut () const
//
...
double TopGluon_Fit_Args::mtdiff_max_cut () const
//
...
int TopGluon_Fit_Args::nkeep () const
//
...
bool TopGluon_Fit_Args::solve_nu_tmass () const
//
...
const Constrained_TopGluon_Args& TopGluon_Fit_Args::constrainer_args () const
//
...
TopGluon_Fit::TopGluon_Fit (const TopGluon_Fit_Args& args,
                  double lepw_mass,
                  double hadw_mass,
                  double top_mass)
//
...
double TopGluon_Fit::fit_one_perm (Lepjets_Event& ev,
                              bool& nuz,
                              double& umwhad,
                              double& utmass,
                              double& mt,
...
//
{
  ...
  if (_args.print_event_flag()) {
    cout << "TopGluon_Fit::fit_one_perm() : Before fit:\n";
  ...
  if (_hadw_mass > 0 && test_for_bad_masses (ev, _args, umwhad,
                                             umthad, umtlep))
  {
    cout << "TopGluon_Fit: bad mass comb.\n";
    return -999;
  }
  ...
  // Trace, if requested.
  if (_args.print_event_flag()) {
    cout << "TopGluon_Fit::fit_one_perm() : After fit:\n";
...
Fit_Results TopGluon_Fit::fit (const Lepjets_Event& ev)
//
...
//  if (_args.do_TopGluon_flag() && ev.njets() >= 6) {
...
//  }
  ...
  do {
    ...
    std::cout<<"list_flags "<< list_flags.size() <<std::endl;
    ...
    // Tracing.
    cout << "TopGluon_Fit::fit(): Before fit: (";
    for (vector<int>::size_type i=0; i < jet_types.size(); i++) {
        if (i) cout << " ";
        cout << jet_types[i];
    }
    ...
    if (_args.print_event_flag()) {
        cout << "TopGluon_Fit::fit(): After fit:\n";
        char buf[256];
...
/**
    @brief Output stream operator, print the content of this TopGluon_Fit object
    to an output stream.

    @param s The output stream to which to write.

    @param fitter The instance of TopGluon_Fit to be printed.
 */
std::ostream& operator<< (std::ostream& s, const TopGluon_Fit& fitter)
//
...
const TopGluon_Fit_Args& TopGluon_Fit::args() const
{
    return _args;
}
